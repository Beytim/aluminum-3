...
import {
  ...
  sampleEnhancedPOs, sampleReorderSuggestions,
  calculateProcurementStats,
  sampleEnhancedSuppliers,
...
export default function Procurement() {
  // Suppliers come from the DB now (read-only here — manage them at /suppliers)
  const { data: dbSuppliers = [] } = useSuppliers();

  // POs still use local storage (will be migrated later)
  ...
  return (
    <div className="space-y-4">
      ...
      <AddPurchaseOrderDialog open={addPOOpen} onOpenChange={o => { setAddPOOpen(o); if (!o) setPreSelectedSupplier(''); }} onAdd={po => { setPurchaseOrders(prev => [...prev, po]); toast({ title: "PO Created", description: po.poNumber }); }} suppliers={localSuppliers} poCount={purchaseOrders.length} preSelectedSupplierId={preSelectedSupplier} />
