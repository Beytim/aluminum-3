...
import { calculateQuoteStats, formatETBCompact } from "@/data/enhancedQuoteData";
...
export default function Quotes() {
  ...
  const handleClone = (q: EnhancedQuote) => {
    ...
    const cloned: EnhancedQuote = {
      ...
      status: 'Draft', version: 'v1', quoteDate: now,
      expiryDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      createdAt: now, updatedAt: now, isConverted: false, isExpired: false,
      projectId: undefined, convertedDate: undefined,
    ...
    addQuote.mutate(cloned);
    toast({ title: "Cloned", description: `${q.quoteNumber} cloned.` });
  };

  const handleBulkDelete = () => {
    selectedIds.forEach(id => deleteQuote.mutate(id));
  ...
  const handleExportPDF = () => {
    ...
    generateReportPDF("Quote Report",
      ...
      data.map(q => [q.quoteNumber, q.title, q.customerName, q.projectName, String(q.items.reduce((s: number, i: any) => s + i.quantity, 0)), formatCurrencyShort(q.total), `${q.profitMargin}%`, q.status, q.expiryDate])
    );
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="p-3">
          ...
          <div className="flex h-3 rounded-full overflow-hidden bg-muted">
            {statusBar.map(sb => sb.count > 0 && (
              <div key={sb.status} className="bg-primary transition-all" style={{ width: `${(sb.count / Math.max(quotes.length, 1)) * 100}%`, opacity: 0.3 + (sb.count / Math.max(quotes.length, 1)) * 0.7 }} title={`${sb.status}: ${sb.count}`} />
            ))}
          </div>
          <div className="flex gap-3 mt-2 flex-wrap">
            {statusBar.map(sb => (
              <div key={sb.status} className="flex items-center gap-1">
                <div className="h-2 w-2 rounded-full bg-primary" style={{ opacity: 0.3 + (sb.count / Math.max(quotes.length, 1)) * 0.7 }} />
      ...
      <AddEnhancedQuoteDialog open={dialogOpen} onOpenChange={setDialogOpen} onAdd={q => addQuote.mutate(q)} customers={customers} products={products} existingCount={quotes.length} />
      <EditEnhancedQuoteDialog open={!!editQuote} onOpenChange={open => { if (!open) setEditQuote(null); }} quote={editQuote} customers={customers} products={products} onSave={updated => updateQuote.mutate(updated)} />
