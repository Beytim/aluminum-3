...
import { getPriorityColor } from "@/data/enhancedMaintenanceData";
...
export function MaintenanceCalendar({ tasks, currentMonth, onMonthChange, onView }: Props) {
  ...
  const byDate = useMemo(() => {
    const map: Record<string, EnhancedMaintenanceTask[]> = {};
    tasks.forEach(t => {
      if (t.scheduledDate.startsWith(`${year}-${String(month + 1).padStart(2, '0')}`)) {
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <h3 className="text-sm font-semibold">{currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}</h3>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onMonthChange(new Date(year, month + 1, 1))}>
              ...
              <div key={day} className={`min-h-[60px] border border-border/50 rounded p-1 ${isToday ? 'bg-primary/5 border-primary/30' : ''}`}>
                ...
                <div className="space-y-0.5 mt-0.5">
                  {dayTasks.slice(0, 2).map(t => (
                    <div key={t.id} className={`text-[8px] px-1 py-0.5 rounded cursor-pointer truncate ${getPriorityColor(t.priority)}`} onClick={() => onView(t)} title={t.title}>
                      {t.taskNumber}
                    </div>
                  ))}
                  {dayTasks.length > 2 && <div className="text-[8px] text-muted-foreground text-center">+{dayTasks.length - 2}</div>}
                </div>
              </div>
            );
          })}
